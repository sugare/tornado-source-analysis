...
serversocket.setblocking(0)         # 由于套接字默认阻塞，因此使用非阻塞（异步）模式是必要的。

epoll = select.epoll()      # 创建一个epoll对象
epoll.register(serversocket.fileno(), select.EPOLLIN)   # 注册对服务器套接字上的读事件的钩子。只要服务器套接字接受套接字连接，就会发生读事件。
print 'sfd', serversocket.fileno()
try:
    connections = {}    # 存储fd:socketobject 键值对， 连接字典将文件描述符（整数）映射到其相应的网络连接对象。
    requests = {}       # 存储fd:response 键值对
    responses = {}      #
    while True:
        events = epoll.poll(1)        # 等待事件的发生，查询epoll对象以了解是否发生了任何感兴趣的事件。参数“1”表示1s无链接，重新轮巡。如果在此查询之前发生了任何感兴趣的事件，则查询将立即返回那些事件的列表。
        print 'evets', events
        for fileno, event in events:      # 事件作为（fileno，事件代码）元组的序列返回。 fileno是文件描述符的同义词，总是一个整数。由于上述监控的是服务器socket的fd，为了避免由于新链接的请求与客户端间的socket冲突，加入if语句
            if fileno == serversocket.fileno():        # 如果在套接字服务器上发生读取事件，则可能已创建新的套接字连接。
                connection, address = serversocket.accept()     # 当外来请求来访时，我们接受请求，建立链接，建立了一个新的通道也就是新的fd
                connection.setblocking(0)       # 将新套接字设置为非阻塞模式。
                print 'conn', connection.fileno()
                epoll.register(connection.fileno(), select.EPOLLIN)     # 注册对新套接字的读（EPOLLIN）事件的兴趣。
                ...
                if EOL1 in requests[fileno] or EOL2 in requests[fileno]:       # 如果刚刚传来的请求中有两个空行，则说明请求已经成功收到。
                    epoll.modify(fileno, select.EPOLLOUT)            # 一旦收到完整的请求，则取消注册读事件的兴趣并注册写（EPOLLOUT）事件的兴趣。当发生写事件时候，将响应数据发送回客户端
    ...
    epoll.unregister(serversocket.fileno())         # 打开套接字连接不需要关闭，因为Python将在程序终止时关闭它们。它们被包括在一个良好的形式。
    epoll.close()
    serversocket.close()
